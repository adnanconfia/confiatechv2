...
@Input() border: border = 'border-0';
@Input() content!: string;
@Input() URL: string='#';
@Input() target: string='self';
@Input() borderRadius: BorderRadius = 'br-12px';
@Input() fontSize:any='fs-16px';
@Input() fontWeight:any='fw-700';
@Input() padding:any='px-md-5  px-1 py-3';
...
export type bgColor =  'bg-white' | 'bg-dark-green' |  'bg-semi-light' | 'bg-ultra-light'| 'bg-danger';
export type Color = 'text-white'| 'text-primary'| 'text-semi-light'|'text-danger'| 'text-danger';
export type border = 'border-0'|'border-primary' | 'border-danger';
